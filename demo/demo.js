...
(function(){
  var loggedIn, pwHash, i, div, entry, showState, state, link, lockScreen, changeHandler, data;
  ...
  window.onkeyup = function(event){
    var ref$;
    if (loggedIn) {
      if ((ref$ = event.code) === 'KeyA' || ref$ === 'ArrowLeft') {
        replaceInnerHTML($('body'), state(data[((--i) % (ref$ = 50) + ref$) % ref$]));
      }
      if ((ref$ = event.code) === 'KeyD' || ref$ === 'ArrowRight') {
        return replaceInnerHTML($('body'), state(data[((++i) % (ref$ = 50) + ref$) % ref$]));
      }
    }
  };
  div = createTag('div');
  ...
  showState = function(stateData){
    return replaceHTML(body, state(data[i++ % 50]));
  };
  state = function(stateData){
    return div({
      'class': 'box'
    }, div({
      'class': 'category'
    }, 'US States'), entry("State", link(stateData[0], stateData[1].replaceAll('_', ' '))), entry("Abbreviation", stateData[2]), entry("Statehood since", stateData[3]), entry("Capital", link(stateData[4], stateData[5].replaceAll('_', ' '))), entry("Capital since", stateData[6]), entry("Area (mi²", stateData[7].split('.')[0]), entry("Area (mi²)", stateData[8].split('.')[0]), entry("City Population", stateData[9]), entry("Metro Population", stateData[10]));
  };
  ...
  changeHandler = async function(){
    ...
    if (hash === pwHash) {
      loggedIn = true;
      return replaceInnerHTML(body, state(data[0]));
    } else {
      $('input').value = '';
      return $('input').placeholder = 'try again';
    }
  };
